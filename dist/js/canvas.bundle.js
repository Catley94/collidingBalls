...
var Particle = /*#__PURE__*/function () {
  ...
  _createClass(Particle, [{
    ...
    value: function update(particles) {
      this.draw(); //loop partciles array

      for (var i = 0; i < particles.length; i++) {
        //if current particle is equial to current particle in the loop, ignore
        if (this === particles[i]) continue; //calculate distance between this 
      ...
      if (_utils__WEBPACK_IMPORTED_MODULE_0___default.a.distance(mouse.x, mouse.y, this.x, this.y) < 120 && this.opacity < 0.4) {
        this.opacity += 0.02;
...
function init() {
  particles = []; //amount of particles

  for (var i = 0; i < numberOfParticles; i++) {
    var radius = 15;
    var color = _utils__WEBPACK_IMPORTED_MODULE_0___default.a.randomColor(colors); //random int from size of partcile (radius) to canvas.width/height - radius, so it will spawn inside the boundries

    var x = _utils__WEBPACK_IMPORTED_MODULE_0___default.a.randomIntFromRange(radius, canvas.width - radius);
    var y = _utils__WEBPACK_IMPORTED_MODULE_0___default.a.randomIntFromRange(radius, canvas.height - radius); //if i is not the first circle created

    if (i !== 0) {
      //loop through partciles all other particles
      for (var j = 0; j < particles.length; j++) {
        //if distance of defined x and defined y above, plus current x and y of j loop, - the radius * 2 is less than 0
        //this is for collision detection, if generated x and y coords are overlapping existing coords already, generate new ones
        if (_utils__WEBPACK_IMPORTED_MODULE_0___default.a.distance(x, y, particles[j].x, particles[j].y) - radius * 2 < 0) {
          //update x and y with new coordinate
